...
const FEATURED_COMPOSERS = [
  {name:'Pixinguinha',asset:'pixinguinha'},
  {name:'Jacob do Bandolim',asset:'jacob'},
  {name:'Waldir Azevedo',asset:'waldir'},
  {name:'Ernesto Nazareth',asset:'ernesto'},
  {name:'Zequinha de Abreu',asset:'zequinha'},
  {name:'Luiz Bonfá',asset:'bonfa'}
];
...
function transposeChord(raw,from,to){
  if(!raw||raw==='%'||raw==='—'||raw==='N.C.'||raw==='NC')return raw;
  const par=raw.match(/^\((.+)\)$/);if(par)return '('+transposeChord(par[1],from,to)+')';
  const m=raw.match(/^([A-G])([#b]?)(.*?)(?:\/([A-G])([#b]?))?$/);if(!m)return raw;
  const semis=distance(from,to),flats=prefersFlats(to),rr=transposeNote(m[1]+(m[2]||''),semis,flats),bb=m[4]?transposeNote(m[4]+(m[5]||''),semis,flats):'';
  return rr+(m[3]||'')+(bb?'/'+bb:'');
}
function parseChart(text){
  ...
  String(text||'').split(/\n/).forEach(raw=>{
    ...
    const h=line.match(/^\[(.+?)\]$/);
    if(h){cur={name:h[1].trim(),measures:[]};sections.push(cur);return}
    if(!cur){cur={name:'A',measures:[]};sections.push(cur)}
    line.split('|').map(x=>x.trim()).filter(Boolean).forEach(cell=>{
      const bits=cell.split('::'),music=bits.shift().trim(),annotation=bits.join('::').trim();
      cur.measures.push({chords:music.split(/\s+/).filter(Boolean),annotation});
    });
  });
  return sections;
}
...
function setupArtwork(){
  if(A.hero)$('heroImage').src=A.hero;
  if(A.classicos)$('featureClassicos').src=A.classicos;
  if(A.encontros)$('featureEncontros').src=A.encontros;
  if(A.regionais)$('featureRegionais').src=A.regionais;
}
function composerCount(name){return allSongs().filter(s=>s.composer.toLowerCase().includes(name.toLowerCase())).length}
function composerAssetFor(text){
  const hay=String(text||'').toLowerCase();
  const found=FEATURED_COMPOSERS.find(c=>hay.includes(c.name.toLowerCase()));
  return found ? A[found.asset] : null;
}
function renderComposers(){
  const cards=$('composerCards'),list=$('composerList');cards.innerHTML='';list.innerHTML='';
  const items=FEATURED_COMPOSERS.map((c,i)=>({...c,count:composerCount(c.name),tone:i%4})).filter(x=>x.count>0);
  $('composerCount').textContent=`${items.length} nomes`;
  items.slice(0,4).forEach(x=>{
    const b=document.createElement('button');b.className='composer-card';
    b.innerHTML=`<span class="composer-portrait">${A[x.asset]?`<img src="${A[x.asset]}" alt="Ilustração de ${esc(x.name)}">`:''}</span><strong>${esc(x.name)}</strong><small>${x.count} ${x.count===1?'música':'músicas'}</small>`;
    b.addEventListener('click',()=>openComposer(x.name));cards.appendChild(b);
  });
  items.forEach(x=>{
    const b=document.createElement('button');b.className='composer-row';
    b.innerHTML=`${A[x.asset]?`<img class="composer-row-img" src="${A[x.asset]}" alt="">`:'<span></span>'}<span><strong>${esc(x.name)}</strong><small>${x.count} ${x.count===1?'música':'músicas'}</small></span><em>${x.count}</em><i>›</i>`;
...
function setHomeView(view){
  homeView=view;document.querySelectorAll('.home-tab').forEach(b=>b.classList.toggle('active',b.dataset.homeView===view));
  $('heroArea').classList.toggle('hidden',view!=='start');
  $('composerSection').classList.toggle('hidden',view==='repertoire'||view==='lists');
  $('featureSection').classList.toggle('hidden',view==='repertoire'||view==='composers');
  $('composerDirectory').classList.toggle('hidden',view!=='composers');
  $('libraryArea').classList.toggle('hidden',view==='composers');
  if(view==='lists'){currentFilter='favorites';document.querySelectorAll('.filter').forEach(b=>b.classList.toggle('active',b.dataset.filter==='favorites'))}
  else if(view==='repertoire'){currentFilter='all';document.querySelectorAll('.filter').forEach(b=>b.classList.toggle('active',b.dataset.filter==='all'))}
  renderLibrary();
}
...
function renderLibrary(){
  const q=$('searchInput').value.toLowerCase().trim(),favs=favorites(),set=setlist(),songs=allSongs().filter(s=>filterSong(s,q,favs,set));
  $('songCount').textContent=`${songs.length} ${songs.length===1?'música':'músicas'}`;
  const list=$('songList');list.innerHTML='';
  if(!songs.length){list.innerHTML='<div class="empty"><h3>Nada por aqui.</h3><p>Mude o filtro, procure outro nome ou use o botão + para criar sua cifra.</p></div>';return}
  songs.forEach(s=>{
    const card=document.createElement('div');card.className='song-card';card.tabIndex=0;card.setAttribute('role','button');card.setAttribute('aria-label',`Abrir ${s.title}`);
    card.innerHTML=`<div class="song-cover"><span class="song-key">${esc(s.baseKey)}</span></div><div class="song-copy"><div class="song-name">${esc(s.title)}${s.official?'<span class="badge">BASE</span>':s.personalBase?'<span class="badge">SEU</span>':''}</div><div class="song-composer">${esc(s.composer)}</div><div class="song-meta">${esc(s.meter)} · ${esc(s.status||'Minha versão')}</div></div><div class="song-actions"><button class="icon-mini fav" aria-label="Favorito">${contains(favs,s.id)?'★':'☆'}</button><button class="icon-mini set" aria-label="Roda de hoje">${contains(set,s.id)?'✓':'＋'}</button><button class="icon-mini open" aria-label="Abrir">›</button></div>`;
    const open=()=>openSong(s);card.addEventListener('click',open);card.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open()}});
    card.querySelector('.fav').addEventListener('click',e=>{e.stopPropagation();toggleId('roda.favorites',s.id)});
    card.querySelector('.set').addEventListener('click',e=>{e.stopPropagation();toggleId('roda.setlist',s.id)});
    card.querySelector('.open').addEventListener('click',e=>{e.stopPropagation();open()});list.appendChild(card);
  });
}
...
function renderChart(container,song){
  container.innerHTML='';
  song.sections.forEach((sec,si)=>{
    const badge=sectionBadge(sec,si),tone=sectionTone(si,badge),repeat=sectionRepeat(song,sec,badge),block=document.createElement('section');
    block.className=`chart-part ${tone}`;const measures=sec.measures;
    block.innerHTML=`<header class="chart-part-head"><span class="part-badge">${esc(badge)}</span><div><strong>${esc(sec.name)}</strong><small>${measures.length} ${measures.length===1?'compasso':'compassos'}</small></div>${repeat?`<em>${repeat}x</em>`:''}</header><div class="part-measures"></div>`;
    const grid=block.querySelector('.part-measures');
    measures.forEach((m,mi)=>{const cell=document.createElement('div');cell.className='measure-line';cell.innerHTML=`<span class="measure-no">${mi+1}</span><div class="chords">${displayChords(m,measures[mi-1],song)}</div>${m.annotation?`<div class="annotation">${esc(m.annotation)}</div>`:''}`;grid.appendChild(cell)});
...
function renderDetail(){
  $('detailTitle').textContent=selected.title;$('detailComposer').textContent=selected.composer;
  const art=composerAssetFor(selected.composer),artEl=$('detailComposerArt');if(art){artEl.src=art;artEl.classList.remove('hidden')}else{artEl.removeAttribute('src');artEl.classList.add('hidden')}
  const tags=$('detailTags');tags.innerHTML='';['Choro',selected.meter,selected.form||'Instrumental'].forEach(v=>{const s=document.createElement('span');s.textContent=v;tags.appendChild(s)});
  const k=displayKey(targetRoot,selected.baseKey);$('currentKey').textContent=k;$('baseKeyLabel').textContent='Tom-base: '+selected.baseKey;$('statusLabel').textContent=selected.status||'Minha versão';renderKeyStrip();renderChart($('chart'),selected);renderSources();updateDetailButtons();
}
...
function renderRodaPart(){
  if(!selected)return;const sections=selected.sections,sec=sections[rodaSectionIndex],badge=sectionBadge(sec,rodaSectionIndex),repeat=sectionRepeat(selected,sec,badge);
  $('rodaPartBadge').textContent=badge;$('rodaProgress').textContent=`${rodaSectionIndex+1} / ${sections.length}`;$('rodaComposer').textContent=selected.composer;$('rodaKey').textContent=displayKey(targetRoot,selected.baseKey);
  const grid=$('rodaPartGrid');grid.innerHTML='';grid.className=`roda-part-grid ${sectionTone(rodaSectionIndex,badge)} ${sec.measures.length>32?'dense':''}`;
  sec.measures.forEach((m,mi)=>{const cell=document.createElement('div');cell.className='roda-measure';const chords=displayChords(m,sec.measures[mi-1],selected);cell.innerHTML=`<span class="roda-no">${mi+1}</span><div class="roda-chords">${chords}</div>${m.annotation?`<div class="roda-annotation">${esc(m.annotation)}</div>`:''}`;grid.appendChild(cell)});
  const r=$('rodaRepeat');r.classList.toggle('hidden',!repeat);r.textContent=repeat?`⟲ repetir ${repeat}x`:'';
  const dots=$('rodaDots');dots.innerHTML='';sections.forEach((_,i)=>{const d=document.createElement('button');d.setAttribute('aria-label',`Parte ${i+1}`);if(i===rodaSectionIndex)d.classList.add('active');d.addEventListener('click',()=>{rodaSectionIndex=i;renderRodaPart()});dots.appendChild(d)});
  $('rodaPrev').disabled=sections.length<2;$('rodaNext').disabled=sections.length<2;
}
...
setupArtwork();renderComposers();renderLibrary();previewEditor();
if('serviceWorker'in navigator){window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('./sw.js?v=5');reg.update()}catch{}})}
